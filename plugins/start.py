...
async def batch_handler(client,message):
    ...
    try:
        ...
        for m in range(Config.START_FROM,last_msg_id):
            ...
            try:
                ...
                await message.reply_text(f"this file id --- {msg.id} can't be rename due to errror ----- \n{e}")
                failed+=1 
                failed_id.append(msg.id)
                await fwd.delete()
                await mes.delete()
            if total % 10 == 0:
...
async def send_doc(client,message):
       ...
       if update_channel :
       	try:
       		...
       		await message.reply_text("**__You are not subscribed my channel__** ",reply_to_message_id = message.id, reply_markup = InlineKeyboardMarkup([ [ InlineKeyboardButton("Support 🇮🇳" ,url=f"https://t.me/{update_channel}") ]   ]))
       		return
       date = message.date
